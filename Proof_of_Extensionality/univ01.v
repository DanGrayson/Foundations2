...
Definition eqweqmap (T1:UU0) (T2:UU0) : (uu1.paths T1 T2) -> (weq T1 T2).
Proof. intros. induction X. apply idweq. Defined. 

Axiom univalenceaxiom: forall T1:UU0, forall T2:UU0, uu1.isweq (uu1.paths T1 T2) (weq T1 T2) (eqweqmap T1 T2).
 
Definition weqtopaths (T1:UU0)(T2:UU0)(w: weq T1 T2) : uu1.paths T1 T2 := uu1.invmap _ _ (eqweqmap T1 T2) (univalenceaxiom T1 T2) w.


Definition weqpathsweq (T1:UU0)(T2:UU0)(w: weq T1 T2):uu1.paths (eqweqmap _ _ (weqtopaths _ _ w)) w := uu1.weqfg _ _ (eqweqmap T1 T2) (univalenceaxiom T1 T2) w.
...
Axiom weqtopaths0: forall (T1:UU0)(T2:UU0)(w:weq T1 T2), uu1.paths T1 T2.

Axiom weqpathsweq0: forall (T1:UU0)(T2:UU0)(w:weq T1 T2), uu1.paths (eqweqmap _ _ (weqtopaths0 _ _ w)) w.

Theorem univfromtwoaxioms: forall T1:UU0, forall T2:UU0, uu1.isweq (uu1.paths T1 T2) (weq T1 T2) (eqweqmap T1 T2).
Proof. intros. set (P1:= fun XY: uu1.dirprod UU0 UU0 => (match XY with uu1.tpair X Y => uu1.paths X Y end)). set (P2:= fun XY: uu1.dirprod UU0 UU0 => match XY with uu1.tpair X Y => weq X Y end). set (Z1:= uu1.total2 _ P1). set (Z2:= uu1.total2 _ P2). set (f:= (uu1.totalfun _ _ _ (fun XY:uu1.dirprod UU0 UU0 => (match XY with uu1.tpair X Y => eqweqmap X Y end))): Z1 -> Z2). set (g:=  (uu1.totalfun _ _ _ (fun XY:uu1.dirprod UU0 UU0 => (match XY with uu1.tpair X Y => weqtopaths0 X Y end))): Z2 -> Z1). set (s1:= (fun X Y :UU0 => fun w: weq X Y => uu1.tpair _ P2 (uu1.dirprodpair X Y) w)). set (efg:= (fun a:_ => match a as a' return (uu1.paths (f (g a')) a') with uu1.tpair (uu1.tpair X Y) w => (uu1.maponpaths (s1 X Y) _ _ (weqpathsweq0 X Y w)) end)). 

set (h:= fun a1:Z1 => (uu1.pr21 _ _ (uu1.pr21 _ _ a1))).
assert (egf0: forall a1:Z1, uu1.paths (uu1.pr21 _ _ (g (f a1))) (uu1.pr21 _ _ a1)). intro. apply uu1.idpath.  
assert (egf1: forall a1 a1':Z1, uu1.paths (uu1.pr21 _ _ a1') (uu1.pr21 _ _ a1) -> uu1.paths a1' a1). intros.  set (X':= uu1.maponpaths (uu1.pr21 _ _ ) _ _ X). 
...
Lemma isweqtransportf10 (X:UU1)(P:X -> UU0)(x:X)(x':X)(e:uu1.paths x x'): isweq _ _ (uu1.transportf X P x x' e).
...
Lemma l1  (X0:UU0)(X0':UU0)(ee: uu1.paths X0 X0')(P:UU0 -> UU0)(pp': P X0')(R: forall X:UU0, forall X':UU0, forall w: weq X X', P X' -> P X)(r: forall X:UU0, forall p:P X, paths (R X X (idweq X) p) p):paths (R X0 X0' (eqweqmap _ _ ee) pp') (uu1.transportb UU0 P X0 X0' ee pp').
Proof. intro. intro. intro. intro. intro. induction ee. simpl. intro. intro. apply r. Defined.


Theorem weqtransportb  (P:UU0 -> UU0)(R: forall (X X':UU0) (w: weq X X'), P X' -> P X)(r: forall X:UU0, forall p:P X, paths (R X X (idweq X) p) p): forall (X X':UU0) (w: weq X X') (p':P X'), paths (R X X' w p') (uu1.transportb UU0  P X X' (weqtopaths _ _ w) p').  
Proof. intros. set (uv:=weqtopaths _ _ w).   set (v:=eqweqmap _ _ uv). 

assert (e:uu1.paths v w). unfold weqtopaths in uv.  apply (uu1.weqfg  (uu1.paths X X') (weq X X')  (eqweqmap X X') (univalenceaxiom X X') w).

assert (ee:uu1.paths (R X X' v p') (R X X' w p')). set (R':= fun vis:weq X X' => R X X' vis p'). assert (ee':uu1.paths (R' v) (R' w)). apply (uu1.maponpaths R' _ _ e). assumption.
...
Lemma eqcor0 (X X':UU0)(w: weq X X')(Y:UU0)(f1 f2:X'->Y):  (paths (fun x:X => f1 (w x))  (fun x:X => f2 (w x))) -> paths f1 f2. 
Proof. intros. apply (pathsweq2 _ _ (fun f:X'->Y => (fun x:X => f (w x))) (isweqcompwithweq _ _ w Y) f1 f2). assumption.  Defined. 


Lemma apathpr1topr2 (T:UU0) : paths (fun z: pathsspace T => pr21 _ _ z) (fun z: pathsspace T => pr21 _ _ (pr22 _ _ z)).
Proof. intro. apply (eqcor0 _ _  (weqpair (deltap T) (isweqdeltap T)) _ (fun z: pathsspace T => pr21 _ _ z) (fun z: pathsspace T => pr21 _ _ (pr22 _ _ z))  (idpath (fun t:T => t))). Defined.     


Theorem funextfun (X:UU0)(Y:UU0)(f1:X->Y)(f2:X->Y)(e: forall x:X, paths (f1 x) (f2 x)): paths f1 f2.
Proof. intros. set (f:= (fun x:X => pathsspacetriple Y (f1 x) (f2 x) (e x))).  set (g1:= (fun z:pathsspace Y => pr21 _ _ z)). set (g2:= fun z: pathsspace Y => pr21 _ _ (pr22 _ _ z)). assert (e': paths g1 g2). apply (apathpr1topr2 Y). assert (ee:paths (fun x:X => f1 x) (fun x:X => f2 x)). apply (maponpaths2b _ _ _ f g1 g2 e').  apply etacoronpaths.  assumption. Defined. 
